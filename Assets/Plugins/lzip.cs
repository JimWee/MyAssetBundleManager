...
public class lzip
{
    ...
    [DllImport("__Internal")]
	private static extern int zipEX(string zipArchive, string outPath, IntPtr progress, IntPtr FileBuffer, int fileBufferLength);
    [DllImport("__Internal")]
    private static extern int zipEntry(string zipArchive, string arc_filename, string outpath, IntPtr FileBuffer, int fileBufferLength);
#endif
    ...
    public static int extract_entry(string zipArchive, string arc_filename, string outpath, byte[] FileBuffer = null)
    {
		int res;
			...
			res = zipEntry(@zipArchive, arc_filename, outpath, IntPtr.Zero, 0);
    ...
    public static int decompress_File(string zipArchive, string outPath, int[] progress, byte[] FileBuffer = null)
    {
		...
		res = zipEX(@zipArchive, @outPath, ibuf.AddrOfPinnedObject(), IntPtr.Zero, 0);
		ibuf.Free();
		return res;
    }
